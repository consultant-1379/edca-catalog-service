...
# currently this test file will push & pull 2k files to bdr in the interval of every 5 mins(totally 10 mins)
...
class helperClass(SequentialTaskSet):
    ...
    def upload_files(self):
        ...
        ccLogs=subprocess.run('kubectl exec ' + ccPodName +' -n '+ccNameSpace+' -- bash -c "java -jar custom-collector.jar --spring.config.additional-location=config.properties --edca.nameSpace='+
           bdrNameSpace+' --edca.test.counter=2000 --automation"',capture_output=True, shell=True,encoding='UTF-8')
        with open(os.path.join(reportDir,"bdr&kafka_continous_load_test.txt"), "a") as file: 
            file.write("\n"+str(ccLogs.stdout))
        if("Uploading file to BDR: success" in str(ccLogs.stdout)):
            print('uploading 2k files:success')
            logger.info('uploaded 2k files:success'+str(ccLogs.stdout))
        else:
            print('uploading 2k files:failed',str(ccLogs.stdout))
            logger.critical('uploaded 2k files:failure'+str(ccLogs.stdout))
    ...
    def download_files(self):
        ...
        drgLogs=subprocess.run('kubectl exec ' + drgPodName +' -n '+drgNameSpace+' -- bash -c "./auto1.sh"',shell=False,encoding='UTF-8',timeout=30)
        with open(os.path.join(reportDir,"bdr&kafka_continous_load_test.txt"), "a") as file1: 
            file1.write("\n"+str(drgLogs))
        if("downloaded not Successfully" in str(drgLogs) and "Downloading object from BDR started" in str(drgLogs)):
            print('downloading 2k files:success')
            logger.info('downloading 2k files:success'+str(drgLogs))
        else:
            print('downloading 2k files:failed',str(drgLogs))
            logger.critical('downloading 2k files:failure'+str(drgLogs))

    @task()
    def stopTask(self):
        logger.info('stopping task -> counter'+self.counter)
        if self.counter==2:
