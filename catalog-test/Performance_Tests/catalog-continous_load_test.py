from locust import SequentialTaskSet,wait_time,constant,task,events,User
...
def on_test_start(environment, **kwargs):
    logger.info('Starting Catalog Continous Load-Test')
...
# bulk-data-repository test cases
    @task()
    def postBulkDataRepository(self):
        self.run_counter=self.run_counter+ 1
        for lp in range(self.start_var_loop,self.range_loop):
                ...
                with open(os.path.join(reportDir,"catalog-continous_load_test.txt"), "a") as file: 
                    file.write("\n"+output1)
...
#data-provider-type test cases
    @task()
    def postdataProviderType(self):
        self.run_counter=self.run_counter+ 1
        for lp in range(self.start_var_loop,self.range_loop):
...
# FileFormat test cases
    ...
    def postFileFormatData(self):
        ...
        for lp in range(self.start_var_loop,self.range_loop):
                ...
                for k in tmp.keys():
                    if(k=='dataProviderTypeId' or k=='notificationTopicId' or k=='bulkDataRepositoryId'):
                        tmp[k] =lp+1
                    if(k=='dataCollectorId'):
                        tmp[k] =self.collectorIds[lp]
    ...
    def getFileFormatData(self):
        ...
        for lp in range(self.start_var_loop,self.range_loop):
                ...
                if '200' not in output1:
                    ...
                    logger.info('file-format: GET: success'+ output1)
                time.sleep(1)
   
    @task()
    def stopTask(self):
        with open(os.path.join(reportDir,"catalog-continous_load_test.txt"), "a") as file: 
            file.write("\n End of "+str(self.run_counter/13)+" round\n")
        if self.run_counter==13:
            truncate_tables()
